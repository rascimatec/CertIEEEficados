...
class PreencherCertificados:
  ...
  def setFont(self):
    while True:
      self.y1 = float(input("Y (uma linha): "))
      self.y2 = float(input("Y (mais de uma linha): "))
      self.width = int(input("Comprimento: "))
      self.font_size = int(input("Tamanho da fonte: "))
      self.espacamento = int(input("Espaçamento: "))
      self.font = ImageFont.truetype(font="_recursos/fonte.ttf", size=font_size)
